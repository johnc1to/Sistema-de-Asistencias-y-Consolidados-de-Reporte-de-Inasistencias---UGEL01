                ...
                ?>
                    <tr>
                        ...
                        ?>
                            ...
                            <td rowspan="<?=($casilla_detalle)?count($casilla_detalle):1?>" style="vertical-align: middle;">
                                <input class="iexp<?=$i?>_doc" type="checkbox" value="<?=$doc[0]['iddocumento']?>" <?=$disabled?> <?=($casilla_detalle)?'':'checked'?>> <?=$doc[0]['nombre']?>
                    ...
                    <?php
                        ...
                        <tr>
                            <td rowspan="<?=($casilla_detalle)?count($casilla_detalle):1?>"> <div style="text-align: center;font-weight:bolder;font-size:20px;"><?=($casilla_detalle)?'SI':'NO'?></div> </td>
                            <td rowspan="<?=($casilla_detalle)?count($casilla_detalle):1?>" style="vertical-align: middle;">
                                <input class="iexp<?=$i?>_doc" type="checkbox" value="<?=$doc[$k]['iddocumento']?>" <?=$disabled?> <?=($casilla_detalle)?'':'checked'?>> <?=$doc[$k]['nombre']?>
...
function notificararregloaguardar(nro=0){
    ...
    if(arregloaguardar.length>nro){
        ...
        $("#notificacionMasivo .modal-content").load('https://aplicacion.ugel01.gob.pe/public/buscarMultipleNotificaciones?idespecialista=<?=$idespecialista?>&idreclamo='+lista_idreclamo.join());
