...
trait RefirmaInvokerNoPatrimonialTraits {

    protected function fileDownloadUrl($ruta=null)
    {
        if($ruta->numero_firmas == 0)
        {
            return "https://siguhs.ugel01.gob.pe/movimiento_no_patrimonial/".$ruta->correlativo.".pdf";
        }else{
            return "https://siguhs.ugel01.gob.pe/movimiento_no_patrimonial/firmados/".$ruta->correlativo."_".$ruta->numero_firmas.".pdf";
        }
    }
    ...
    private function CalcularXYSimple1($movimiento,$persona){
        ...
        {
            $tipo2 = PdfFirmaDesplazamientoNoPatrimonial::whereCorrelativoId($movimiento->correlativo)->wherePersonaId($persona->CodPer)
            ->whereEstado(1)->whereTipo(2)->first();
            $tipo3= PdfFirmaDesplazamientoNoPatrimonial::whereCorrelativoId($movimiento->correlativo)->wherePersonaId($persona->CodPer)
            ->whereEstado(1)->whereTipo(3)->first();
            $tipo4= PdfFirmaDesplazamientoNoPatrimonial::whereCorrelativoId($movimiento->correlativo)->wherePersonaId($persona->CodPer)
    ...
    {
        $pagina =  BienesMovimientoNoPatrimonial::whereCorrelativo($request->input("id"))->whereEstado(1)->first();
        ...
        $param ='{
            ...
            "fileUploadUrl":"https://siguhs.ugel01.gob.pe/laravel-no-patrimonial",
