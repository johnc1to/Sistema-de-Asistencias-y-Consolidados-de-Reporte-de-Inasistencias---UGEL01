...
use App\Http\Traits\RefirmaInvokerTraits;
...
class RefirmaInvokerController extends Controller
{
    use RefirmaInvokerTraits;
    ...
    public function update_firmas(Request $request)
    {
        ...
        try {
            $pagina =   DB::connection("bienes")->table("bienes_movimiento1")->whereCorrelativo($request->input("id"))->whereEstado(1)->first();
            DB::connection("bienes")->table("bienes_movimiento1")->whereCorrelativo($request->input("id"))->whereEstado(1)
            ->update(["numero_firmas"=>$pagina->numero_firmas+1]);

            $tipo2 = PdfFirmaDesplazamiento::whereCorrelativoId($request->input("id"))->wherePersonaId($userSiguhs->CodPer)
            ->whereEstado(1)->whereTipo(2)->first();
            $tipo3= PdfFirmaDesplazamiento::whereCorrelativoId($request->input("id"))->wherePersonaId($userSiguhs->CodPer)
            ...
            PdfFirmaDesplazamiento::create(['correlativo_id'=>$request->input("id"), 'persona_id'=>$userSiguhs->CodPer,
            'numero_firma'=>$pagina->numero_firmas+1,"tipo"=>$tipo]);

             if($pagina->id_movimiento == 4 and ($pagina->numero_firmas+1) == 4)
             {
                $dato = DB::connection("bienes")->table("bienes_movimiento1")->selectRaw("btd.id")
                ->join("bien_temporal_devolucion as btd","btd.id_bien_movimiento","=","bienes_movimiento1.id")
                ->whereNull("btd.fecha_recepcion")->where("bienes_movimiento1.id_movimiento",4)
                ->whereCorrelativo($pagina->correlativo)->get()->pluck("id")->toArray();
                $bienes = DB::connection("bienes")->table("bienes_movimiento1")->selectRaw("id_bien")
                ->whereCorrelativo($pagina->correlativo)
                ->whereEstado(1)->get()->pluck("id_bien")->toArray();
                 BienTemporalDevolucion::whereIn("id",$dato)->update(["fecha_recepcion"=>date("Y-m-d")]);
