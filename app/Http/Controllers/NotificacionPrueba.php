...
use App\Models\Mantenimientoprueba;
use App\Models\Reclamosprueba;
use App\Models\Documentoprueba;
use App\Models\Receptorprueba;
use App\Models\Casillaprueba;
use App\Models\Casilladetalleprueba;
use App\Models\Correlativoprueba;
...
class NotificacionPrueba extends Controller
{
    public function prueba(){
       $data = Casilladetalleprueba::where(['iddocumento'=>8])->get()->toArray();
       dd($data);
    }
    ...
    public function buscarMultipleNotificaciones(Request $request){
        ...
        $data=Reclamosprueba::whereIn('idreclamo',$idreclamo)->join('receptor','reclamos.idreceptor','=','receptor.idreceptor')->select('idreclamo','cod_reclamo','resumen_pedido','tipo_exp','nombres','apellido_paterno','apellido_materno','tipodocumento','reclamos.idreceptor')->get()->toArray();
        if($data){
            for ($i=0; $i < count($data); $i++) {                
                $doc = Documentoprueba::where(['estado'=>1,'adj_idreclamo'=>$data[$i]['idreclamo']])->select('*',DB::raw("IF(estado_firmado=1,REPLACE(archivo,'/cargados/','/firmados/'), archivo) as archivo"))->get()->toArray();
                $data[$i]['nrow'] = 0;
                if($doc){
                    for ($k=0; $k < count($doc); $k++) {
                        $iddocumento = $doc[$k]['iddocumento'];
                        $doc[$k]['casilla_detalle'] = Casilladetalleprueba::where(['casilla_detalle.estado'=>1,'casilla_detalle.iddocumento'=>$iddocumento])
        ...
        $info['data']           = $data;
        return view('notificacion/buscarMultipleNotificacionesprueba',$info);
    }

    public function pstrpos(){
        /*
        $data=Contactoprueba::where('institucion', 'like', '%' . 'SASAKAWA' . '%')
        ...
        */
        ...
        if($data){
        $lista = DB::connection('notificacionprueba')->select("SELECT idreceptor, CONCAT(nombres,' ',apellido_paterno,' ',apellido_materno) as nombrecompleto FROM receptor WHERE documento IN(".$data[0]->dni.") and estado=1");
        }else{
        $lista=false;
        }
        dd($lista);
    ...
    public function buscarciudadano(Request $request){
        ...
        $lista = DB::connection('notificacionprueba')->select("SELECT idreceptor, CONCAT(nombres,' ',apellido_paterno,' ',apellido_materno) as nombrecompleto FROM receptor WHERE CONCAT(nombres,' ',apellido_paterno,' ',apellido_materno) LIKE '%".$request->input('valor')."%' and estado=1 and etapa_de_registro=2");
        }
    ...
    public function notificarciudadano(Request $request){
        header('Access-Control-Allow-Origin: *');        
        $reclamo = Reclamosprueba::where(['estado'=>1,'idreclamo'=>$request->input('idreclamo')])->select('cod_reclamo','idreclamo','tipo_exp')->get()->first();
        $idmantenimiento = ($request['idmantenimiento'])?$request['idmantenimiento']:(($reclamo['tipo_exp']=='INTERNO')?932:46);//Local: 925    Nube: 932
        $mantenimiento = Mantenimientoprueba::where(['estado'=>1,'idmantenimiento'=>$idmantenimiento])->select('abreviatura','descripcion')->get()->first();
        ...
        $idcasilla = Casillaprueba::insertGetId($casilla);
        $doc   = $request->input('doc');
        $recep = $request->input('receptor');
        for ($i=0; $i < count($doc); $i++) {
            $documento = Documentoprueba::where(['estado'=>1,'iddocumento'=>$doc[$i]])->select('*',DB::raw("IF(estado_firmado=1,REPLACE(archivo,'/cargados/','/firmados/'), archivo) as archivo"))->get()->first();
            for ($k=0; $k < count($recep); $k++) {
                $receptor = Receptorprueba::where(['estado'=>1,'idreceptor'=>$recep[$k]])->get()->first();
                ...
                $correlativo = Correlativoprueba::where(['estado'=>1,'tipo'=>'acuse_recibido','anio'=>date('Y')])->select('numero','tipo','anio')->first();
                DB::connection('notificacionprueba')->table('correlativo')->where(['estado'=>1,'tipo'=>'acuse_recibido','anio'=>date('Y')])->increment('numero');
                ...
                Casilladetalleprueba::insert($detalle);
                //$this->siic01_enviarcorreo($casilla['asunto'],$receptor['correo'],$casilla['cuerpo']);
                $receptor['correo'] = 'locacion_jmmj@ugel01.gob.pe';
                //$this->enviarmail($casilla['asunto'],$receptor['correo'],$casilla['cuerpo']);
                $send = new MailerController();
                $send->sendMail($casilla['asunto'],$receptor['correo'],$casilla['cuerpo']);
            }
        }
        //Añadir el campo updated_at en la tabla reclamos, el update en laravel utiliza ese campo por defecto
        Reclamosprueba::where(['estado'=>1,'idreclamo'=>$request->input('idreclamo')])->update(['etapa'=>(($reclamo['tipo_exp']=='INTERNO')?'COMUNICADO':'NOTIFICADO')]);
    }
