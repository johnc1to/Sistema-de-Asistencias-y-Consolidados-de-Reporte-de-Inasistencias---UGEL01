...
class Login extends Controller{
    ...
    public function ingreso_dir(Request $request){
        ...
        if($datos){
            ...
            $datos['modulos_adicionales'] = $this->modulos_adicionales($datos['id_contacto'],$datos['modulos_por_defecto'],0,0," AND tipo='DIR' AND grupo IS NULL ");
            $grupos_adicionales = $this->grupos_adicionales($datos['id_contacto'],0,0," AND tipo='DIR' ");
            if($grupos_adicionales){
            for ($i=0; $i < count($grupos_adicionales); $i++) {
                if($grupos_adicionales[$i]['grupo']){
                    $grupos_adicionales[$i]['modulos'] = $this->modulos_adicionales($datos['id_contacto'],$datos['modulos_por_defecto'],0,0," AND tipo='DIR' AND grupo = '".$grupos_adicionales[$i]['grupo']."'");
                }
            }
            }
    ...
    public function login_admin($usuario,$password){
        ...
        if($datos['pass'] == $password){
            $datos['modulos_adicionales'] = $this->modulos_adicionales($datos['idespecialista'],$datos['modulos_por_defecto'],$datos['id_area'],$datos['id_oficina']," AND tipo='ESP' AND grupo IS NULL ");
            $grupos_adicionales = $this->grupos_adicionales($datos['idespecialista'],$datos['id_area'],$datos['id_oficina']," AND tipo='ESP' ");
            if($grupos_adicionales){
            for ($i=0; $i < count($grupos_adicionales); $i++) {
                if($grupos_adicionales[$i]['grupo']){
                    $grupos_adicionales[$i]['modulos'] = $this->modulos_adicionales($datos['idespecialista'],$datos['modulos_por_defecto'],$datos['id_area'],$datos['id_oficina']," AND tipo='ESP' AND subgrupo IS NULL AND grupo = '".$grupos_adicionales[$i]['grupo']."'");
                    
                    $sub_grupos_adicionales = $this->sub_grupos_adicionales($datos['idespecialista'],$datos['modulos_por_defecto'],$datos['id_area'],$datos['id_oficina'],$grupos_adicionales[$i]['grupo']);
					    if($sub_grupos_adicionales){
					        for ($k=0; $k < count($sub_grupos_adicionales); $k++) {
					            $sub_grupos_adicionales[$k]['modulos'] = $this->modulos_adicionales($datos['idespecialista'],$datos['modulos_por_defecto'], $datos['id_area'], $datos['id_oficina']," AND subgrupo = '".$sub_grupos_adicionales[$k]['subgrupo']."'");
					        }
					    }
    ...
    function grupos_adicionales($idespecialista, $id_area, $id_oficina,$where=''){
    ...
    function sub_grupos_adicionales($idespecialista,$modulos_por_defecto=0, $id_area, $id_oficina,$grupo=false){
